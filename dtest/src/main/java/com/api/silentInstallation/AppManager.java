package com.api.silentInstallation;
            ...
            try {
                ...
                IPackageInstallObserver2 observer = new IPackageInstallObserver2.Stub() {
                    ...
                    public void onPackageInstalled(String packageName, int returnCode, String msg, Bundle extras) {
                        if (packageName != null && returnCode == 1) {
                            Log.e("sdk", "onPackageInstalled onSuccess "+packageName+"_"+returnCode+"_"+msg+"_"+extras);
                            if (listener != null)
                                listener.onSuccess();
                        } else {
                            if (listener != null)
                                listener.onError(new RuntimeException("install error "+returnCode+"-"+msg+"-"+extras));
                        }
                    }
                };
...
//                Log.i("sdk",e.toString());
    ...
    public static boolean openNotificationListenerService(Context context, String serviceName) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR2) {
            Log.e("sdk","NotificationListenerService open failed, need Android 4.3 or high");
            return false;
        }
        if (context.checkPermission("android.permission.WRITE_SECURE_SETTINGS", Process.myPid(), Process.myUid()) == PackageManager.PERMISSION_DENIED) {
            Log.e("sdk","NotificationListenerService open failed");
            return false;
        }

        String services = Settings.Secure.getString(context.getContentResolver(), "enabled_accessibility_services");
        String services2 = Settings.Secure.getString(context.getContentResolver(), "enabled_notification_policy_access_packages");
        Log.e("sdk","=====enabled_notification_policy_access_packages services2 "+services2);
