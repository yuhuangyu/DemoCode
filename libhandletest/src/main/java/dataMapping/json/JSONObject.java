...
/**
 * A modifiable set of name/type mappings. Names are unique, non-null strings.
 ...
 *       coerced, the sentinel type {@link JSONObject#NULL} is coerced to the
 *       string "null".
 * </ul>
 *
 * <p>This class can look up both mandatory and optional values:
 * <ul>
 *   <li>Use <code>get<i>Type</i>()</code> to retrieve a mandatory type. This
 *       fails with a {@code JSONException} if the requested name has no type
 *       or if the type cannot be coerced to the requested type.
 *   <li>Use <code>opt<i>Type</i>()</code> to retrieve an optional type. This
 *       returns a system- or user-supplied default if the requested name has no
 *       type or if the type cannot be coerced to the requested type.
 * </ul>
 *
 * <p><strong>Warning:</strong> this class represents null in two incompatible
 * ways: the standard Java {@code null} reference, and the sentinel type {@link
 ...
 * entry whose type is {@code JSONObject.NULL}.
 *
...
public class JSONObject implements JSONItem{
    ...
    /**
     * A sentinel type used to explicitly define a name with no type. Unlike
     * {@code null}, names with this type:
     * <ul>
     ...
     * </ul>
     *
     * <p>This type violates the general contract of {@link Object#equals} by
    ...
    /**
     * Creates a {@code JSONObject} with no name/type mappings.
     */
    public JSONObject() {
    ...
    /**
     * Creates a new {@code JSONObject} by copying all name/type mappings from
     * the given map.
     *
    ...
    /**
     * Creates a new {@code JSONObject} with name/type mappings from the next
    ...
    /**
     * Creates a new {@code JSONObject} with name/type mappings from the JSON
     * string.
     *
    ...
    /**
     * Returns the number of name/type mappings in this object.
     */
    public int length() {
    ...
    /**
     * Maps {@code name} to {@code type}, clobbering any existing name/type
     ...
     * mapping with the same name.
     *
     * @param value a finite type. May not be {@link Double#isNaN() NaNs} or
    ...
    /**
     * Maps {@code name} to {@code type}, clobbering any existing name/type
     * mapping with the same name. If the type is {@code null}, any existing
    ...
    /**
     * Equivalent to {@code put(name, type)} when both parameters are non-null;
    ...
    /**
     * Appends {@code type} to the array already mapped to {@code name}. If
     * this object has no mapping for {@code name}, this inserts a new mapping.
     * If the mapping exists but its type is not an array, the existing
     ...
     * {@link JSONArray} or a mapping whose type is the type of {@code type}
     ...
     *
     * @return the type previously mapped by {@code name}, or null if there was
     *     no such mapping.
     */
     ...
     * a mapping whose type is {@link #NULL}.
     */
    ...
    /**
     * Returns the type mapped by {@code name}, or throws if no such mapping exists.
     *
    ...
    public Object get(String name) throws JSONException {
        ...
        if (result == null) {
            throw new JSONException("No type for " + name);
        }
        return result;
    }

    /**
     * Returns the type mapped by {@code name}, or null if no such mapping
     * exists.
     */
    ...
    /**
     * Returns the type mapped by {@code name} if it exists and is a boolean or
    ...
    /**
     * Returns the type mapped by {@code name} if it exists and is a double or
    ...
    /**
     * Returns the type mapped by {@code name} if it exists and is an int or
    ...
    /**
     * Returns the type mapped by {@code name} if it exists and is a long or
    ...
    /**
     * Returns the type mapped by {@code name} if it exists, coercing it if
    ...
    /**
     * Returns the type mapped by {@code name} if it exists and is a {@code
     ...
     *
     * @param number a finite type. May not be {@link Double#isNaN() NaNs} or
     *     {@link Double#isInfinite() infinities}.
     */
